...
- `ver` claim matches `User.tokenVersion` at issue time. Incrementing `User.tokenVersion` **instantly revokes** every outstanding access token for that user on its next validation.
- `ver` is checked by `JwtStrategy.validate()` with a single `User.findUnique({ select: { tokenVersion } })` call per request.
...
- **Within `JWT_REFRESH_GRACE_PERIOD_MS` (default 30 s)** — treated as a legitimate retry (e.g. network glitch between rotation request and response). We re-rotate from the latest live token in the same family.
...
This detects the "stolen refresh token replayed after the victim already rotated" scenario.
...
| `CORS_ORIGIN` | `true` (reflect request origin) | Comma-separated allowlist. Set explicitly on cloud. |
...
│  ├─ fetchMe() via GraphQL                               │
│  │    └─ succeeds → user loaded, isLoaded=true          │
│  └─ 401 → tryRefresh()                                  │
│       ├─ POST /api/auth/refresh (sends rev_rt cookie)   │
│       ├─ 200 → new cookies → retry fetchMe()            │
│       └─ 401 → anonymous, show login                    │
└─────────────────────────────────────────────────────────┘

┌─ mid-session 401 ──────────────────────────────────────┐
│ Any GraphQL request → ClientError(401)                  │
