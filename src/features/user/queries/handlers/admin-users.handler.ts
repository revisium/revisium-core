...
} from 'src/features/user/queries/impl/admin-users.query';
import { getOffsetPagination } from 'src/features/share/commands/utils/getOffsetPagination';

@QueryHandler(AdminUsersQuery)
export class AdminUsersHandler
  implements IQueryHandler<AdminUsersQuery, AdminUsersQueryReturnType>
{
  constructor(private readonly prisma: PrismaService) {}

  public async execute({ data }: AdminUsersQuery) {
  ...
  private getWhereInput(data: AdminUsersQuery['data']): Prisma.UserWhereInput {
    return data.search
      ? {
          OR: [
